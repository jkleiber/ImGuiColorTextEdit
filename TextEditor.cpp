    ...
    while (*aValue != '\0')
    {
        ...
        else if (*aValue == '\n')
        {
            if (cindex < (int)mLines[aWhere.mLine].size())
            {
                ...
                newLine.insert(newLine.begin(), line.begin() + cindex,
                               line.end());
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates &aFrom) const
{
    ...
    while (!isword || skip)
    {
        ...
        if (cindex < (int)line.size())
        {
            ...
            if (isword && !skip)
                return Coordinates(at.mLine,
                                   GetCharacterColumn(at.mLine, cindex));
...
void TextEditor::Render()
{
    ...
    if (!mLines.empty())
    {
        ...
        while (lineNo <= lineMax)
        {
            ...
            longest =
                std::max(mTextStart + TextDistanceToLineStart(Coordinates(
            ...
            {
                auto end = ImVec2(lineStartScreenPos.x + contentSize.x +
                                      2.0f * scrollX,
                                  lineStartScreenPos.y + mCharAdvance.y);
            ...
            for (int i = 0; i < line.size();)
            {
                    ...
                    !mLineBuffer.empty())
                {
                    ...
                    drawList->AddText(newOffset, prevColor,
                                      mLineBuffer.c_str());
                ...
                else if (glyph.mChar == ' ')
                {
                    if (mShowWhitespaces)
                    {
                        ...
                        const auto y =
                            textScreenPos.y + bufferOffset.y + s * 0.5f;
                        drawList->AddCircleFilled(ImVec2(x, y), 1.5f,
                                                  0x80808080, 4);
                    }
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
    ...
    if (HasSelection())
    {
            ...
            mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine)
        {
            ...
            if (modified)
            {
                start = Coordinates(start.mLine,
                                    GetCharacterColumn(start.mLine, 0));
                ...
                else
                {
                    ...
                    rangeEnd = Coordinates(end.mLine - 1,
                                           GetLineMaxColumn(end.mLine - 1));
...
void TextEditor::Backspace()
{
    ...
    else
    {
        ...
        if (mState.mCursorPosition.mColumn == 0)
        {
            ...
            for (auto &i : mErrorMarkers)
                etmp.insert(ErrorMarkers::value_type(
                    i.first - 1 == mState.mCursorPosition.mLine ? i.first - 1
                                                                : i.first,
                    i.second));
            mErrorMarkers = std::move(etmp);
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
    ...
    for (int i = aFromLine; i < endLine; ++i)
    {
        ...
        for (auto first = bufferBegin; first != last;)
        {
            ...
            {
                if (mLanguageDefinition.mTokenize(first, last, token_begin,
            ...
            else
            {
                ...
                if (token_color == PaletteIndex::Identifier)
                {
                    ...
                    if (!line[first - bufferBegin].mPreprocessor)
                    {
                        ...
                        else if (mLanguageDefinition.mIdentifiers.count(id) !=
                                 0)
                            token_color = PaletteIndex::KnownIdentifier;
                        else if (mLanguageDefinition.mPreprocIdentifiers.count(
                                     id) != 0)
                    ...
                    else
                    {
                        ...
                        if (singleStartStr.size() > 0 &&
                            currentIndex + singleStartStr.size() <=
                                line.size() &&
                            equals(singleStartStr.begin(), singleStartStr.end(),
                                   from, from + singleStartStr.size(), pred))
                        {
                        ...
                        if (currentIndex + 1 >= (int)endStr.size() &&
                            equals(endStr.begin(), endStr.end(),
                                   from + 1 - endStr.size(), from + 1, pred))
                        {
...
TextEditor::LanguageDefinition::CPlusPlus()
{
    ...
    if (!inited)
    {
                               ...
                               PaletteIndex &paletteIndex) -> bool
        {
            ...
            else if (TokenizeCStyleIdentifier(in_begin, in_end, out_begin,
                                              out_end))
                paletteIndex = PaletteIndex::Identifier;
            else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Number;
            else if (TokenizeCStylePunctuation(in_begin, in_end, out_begin,
                                               out_end))
...
const TextEditor::LanguageDefinition &TextEditor::LanguageDefinition::Python()
{
    ...
    if (!inited)
    {
        ...
        static const char *const identifiers[] = {
            "abort",      "abs",       "acos",       "all",         "any",
            "asdouble",   "asfloat",   "asin",       "asint",       "asint",
            "asuint",     "asuint",    "atan",       "atan2",       "ceil",
            "clamp",      "clip",      "cos",        "cosh",        "countbits",
            "cross",      "ddx",       "ddx_coarse", "ddx_fine",    "ddy",
            "ddy_coarse", "ddy_fine",  "degrees",    "determinant", "distance",
            "dot",        "dst",       "errorf",     "exp",         "exp2",
            "floor",      "fma",       "fmod",       "frac",        "isfinite",
            "isinf",      "isnan",     "ldexp",      "len",         "lerp",
            "lit",        "log",       "log10",      "log2",        "mad",
            "max",        "min",       "modf",       "msad4",       "mul",
            "noise",      "normalize", "pow",        "print",       "radians",
            "rcp",        "reflect",   "refract",    "reversebits", "round",
            "rsqrt",      "saturate",  "sign",       "sin",         "sincos",
